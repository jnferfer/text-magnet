...
var diff = function diff(current, next, field) {
  ...
  var unchanged = lodash.intersectionWith(next, current, lodash.isEqual);
  var updated = lodash.differenceWith(lodash.intersectionWith(next, current, function (a, b) {
    return a[field] === b[field];
  }), unchanged, lodash.isEqual);
  var added = lodash.differenceWith(lodash.differenceWith(next, current, lodash.isEqual), updated, lodash.isEqual);
  return {
    removed: removed,
    unchanged: unchanged,
    updated: updated,
...
var VisGraph = /*#__PURE__*/React.forwardRef(function (_ref, ref) {
  ...
  var edges = useSealedState(function () {
    return new visData.DataSet(graph.edges);
  });
  var nodes = useSealedState(function () {
    return new visData.DataSet(graph.nodes);
  });
  var initialOptions = useSealedState(propOptions);
  var prevNodes = React.useRef(graph.nodes);
  var prevEdges = React.useRef(graph.edges);
  React.useEffect(function () {
    if (lodash.isEqual(graph.nodes, prevNodes.current)) {
      return; // No change!
    }
  ...
  React.useEffect(function () {
    if (lodash.isEqual(graph.edges, prevEdges.current)) {
      return; // No change!
    }
  ...
  React.useEffect(function () {
    ...
    var mergedOptions = lodash.defaultsDeep(lodash.cloneDeep(initialOptions), defaultOptions);
    var newNetwork = new visNetwork.Network(container.current, {
      edges: edges,
      nodes: nodes
    }, mergedOptions);
...
//# sourceMappingURL=react-vis-graph-wrapper.cjs.development.js.map
